...
class Field{
  ...
  void random_drop(float drop_presure){
    int rx = (int)random(n - 10) + 5;
    int ry = (int)random(m - 10) + 5;
    int dx = (int)random(3);
    int dy = (int)random(3);
    for(int x = -dx; x < dx; x++)
      for(int y = -dy; y < dy; y++)
        a[rx + x][ry + y] += drop_presure;
    }
  
  void create_pressure(int x, int y, int max_x, int max_y, float presure){
    int rx= x * n / max_x; //<>// //<>// //<>// //<>//
    int ry = y * m / max_y;
    rx = rx >= n? n -1: rx; 
    ry = ry >= m? m -1: ry; 
