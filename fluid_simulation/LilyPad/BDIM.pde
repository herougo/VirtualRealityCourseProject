...
class BDIM{
  ...
  VectorField u,del,del1,c,u0,ub,wnx,wny,distance,rhoi; // u output, other intermediate
  Field p; // input
  Field color_field;
  Field color_field0;
  int frequency = 10;
  boolean QUICK, mu1=true, adaptive=true, rain=true;
  ...
  BDIM( int n_, int m_, float dt_, Body body, VectorField uinit, float nu_, boolean QUICK_ ){
    ...
    get_coeffs(body);
    //color_field = new Field(n, m, 0, 0);// NEW
    //color_field0 = new Field(n, m, 0, 0);// NEW
  }
  ...
  BDIM( int n, int m, float dt, Body body){this(n,m,dt,body,new VectorField(n+2,m+2,0.5,0.5),0,false);}
...
*/
  ...
  void blood() {
    //color_field.a[p.n / 2 - 20][p.n / 2 - 20] += 20.0;
    color_field.random_drop(random(10.0, 30.0));
  }
  
  void rain(){
    p.random_drop(random(10.0, 30.0));
  }
