...
class FloodPlot{
  ...
  FloodPlot( Window window, String title, float low, float high ){
    ...
    img = new PImage(window.dx,window.dy,RGB);
  ...
  color colorScale( float f ){
    ...
    //print(f, sequential, i, hue.outB(i), hue.outB(-1.0), hue.outB(1.0), hue.outB(0.0), range.in(-1.0), range.in(1.0), range.in(0.0));
    //int b=0/0;
    
    if( sharp & i>0.4 & i<0.6 ) i = 0.5;
    if(sequential){          // blend hue and saturation
      return color(hue.outB(i),sat.out(i),bri.out(i) * alpha);
    } 
    else if(i<0.5) {       // blend from low end to black/white
      if (dark) {return color(hue.outS,sat.outS,(1-2*i)*alpha);}
      else {return color(hue.outS,(0.5-i)*2,1*0);}
    } 
    else {                 // blend from high end to black/white
      if (dark) {return color(hue.outE,sat.outE,(2*i-1)*alpha);}
      else {return color(hue.outE,(i-0.5)*2,1*alpha);}
    }
  } 
